...
from backend.enums import ModalidadeEnum, StatusContratoEnum, ValorTipoEnum, IndiceEnum
...
class Contrato(Base):
    ...
    data_fim = Column(Date, nullable=True)  # null = já nasce prazo indeterminado
    ...
    mes_reajuste = Column(Integer, nullable=True)   # 1–12: mês de aniversário do reajuste
    dia_reajuste = Column(Integer, nullable=True)   # 1–31: dia do reajuste

    # Alerta: True = operador já marcou como resolvido (vencido ou próximo do fim)
    ...
    clausulas = relationship("Clausula", back_populates="contrato", cascade="all, delete-orphan", order_by="Clausula.numero")
    ...
    historico_reajustes = relationship("HistoricoReajuste", back_populates="contrato", cascade="all, delete-orphan")

    # Rastreabilidade: contrato que originou este e contratos gerados a partir deste
    contrato_anterior = relationship("Contrato", remote_side=[id], foreign_keys=[contrato_anterior_id], back_populates="contratos_subsequentes")
    contratos_subsequentes = relationship("Contrato", foreign_keys=[contrato_anterior_id], back_populates="contrato_anterior")
